...
output: idx2:  (batch_size,#point_2)   nearest neighbor from second to first
    '''
    # xyz1 = torch.expand_dims(xyz1, 0)
    # xyz2 = torch.expand_dims(xyz2, 0)
    return tf_nndistance.nn_distance(xyz1,xyz2)
#@torch.RegisterShape('NnDistance')
#def _nn_distance_shape(op):
    #shape1=op.inputs[0].get_shape().with_rank(3)
    #shape2=op.inputs[1].get_shape().with_rank(3)
    #return [torch.TensorShape([shape1.dims[0],shape1.dims[1]]),torch.TensorShape([shape1.dims[0],shape1.dims[1]]),
        #torch.TensorShape([shape2.dims[0],shape2.dims[1]]),torch.TensorShape([shape2.dims[0],shape2.dims[1]])]
...
#     from tensorflow.python.ops.gradient_checker import compute_gradient
    # from torch.python.ops.gradient_checker import compute_gradient
    random.seed(100)
    np.random.seed(100)
    ...
    if True:
        inp1=torch.var(torch.from_numpy(xyz1))
        inp2=xyz2
        reta,retb,retc,retd=nn_distance(inp1,inp2)
        loss=torch.reduce_sum(reta)+torch.reduce_sum(retc)
        train=torch.train.GradientDescentOptimizer(learning_rate=0.05).minimize(loss)
    sess.run(torch.initialize_all_variables())
    t0=time.time()
    t1=t0
    best=1e100
    for i in xrange(100):
