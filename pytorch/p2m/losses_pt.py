...
def laplace_coord(input, lap_idx, block_id, use_cuda=True):
    ...
    vertex = torch.cat(
        (input, torch.zeros(1, 3).cuda()), 0) if use_cuda else torch.cat(
...
def total_pts_loss(pred_pts_list,
                   pred_feats_list,
                   gt_pts,
                   ellipsoid,
                   use_cuda=True):
    ...
    for i in range(3):
        ...
        my_edge_loss += edge_loss(pred_pts_list[i], gt_pts, ellipsoid["edges"],
                                  i, use_cuda)
        my_lap_loss += lap_const[i] * laplace_loss(
            pred_feats_list[i], pred_pts_list[i], ellipsoid["lap_idx"], i,
            use_cuda)
...
def total_img_loss(pred_img, gt_img):
    ...
    my_rect_loss = torch.nn.functional.binary_cross_entropy(pred_img,
                                                            gt_img,
                                                            size_average=False)
