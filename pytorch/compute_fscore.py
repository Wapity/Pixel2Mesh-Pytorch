...
args.add_argument('--fscore_data',
                  help='fscore score data.',
                  type=str,
...
data = DataFetcher(FLAGS.fscore_data)
data.setDaemon(True)
data.start()
data_number = data.number
print('---- Loadind fscore data, {} num samples'.format(data_number))
...
threshold = 0.0001
score_fscore = fscore(dist1, dist2, threshold)[0].mean().item()
print('------> threshold = {}, fscore = {}'.format(threshold, score_fscore))
threshold = 0.0002
score_fscore = fscore(dist1, dist2, threshold)[0].mean().item()
print('------> threshold = {}, fscore = {}'.format(threshold, score_fscore))
