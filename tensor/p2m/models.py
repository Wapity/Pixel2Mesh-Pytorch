...
class GCN(Model):
    ...
    def _loss(self):
        ...
        self.loss += .1 * laplace_loss(self.inputs, self.output1,
                                       self.placeholders, 1)
        self.loss += laplace_loss(self.output1_2, self.output2,
                                  self.placeholders, 2)
        self.loss += laplace_loss(self.output2_2, self.output3,
                                  self.placeholders, 3)
    ...
    def _build(self):
        ...
        self.layers.append(GraphProjection(placeholders=self.placeholders))
        self.layers.append(
            GraphPooling(placeholders=self.placeholders,
                         pool_id=1))  # unpooling
        self.layers.append(
        ...
        def build_cnn18(self):
            ...
            self.loss += tf.add_n(
                tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)) * 0.3
